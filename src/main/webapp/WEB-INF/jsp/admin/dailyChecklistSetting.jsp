                    ...
                    <div class="offcanvas-body pb-5">
                        ...
                        <div class="mt-4" style="border-top:1px dotted #9f9f9f; padding-top:25px;">
                            ...
                            <table class="table">
                                <thead>
                                <tr>
                                    <th style="width:42%;">항목명</th>
                                    <th style="width:16%;">입력형식</th>
                                    <th style="width:14%;">필수</th>
                                    <th style="width:14%;">사용</th>
...
<script>
    ...
    function loadChecklists(page) {
        ...
        $.ajax({
            ...
            error: function () {
                showMsg('error', '오류', '체크리스트 목록 조회 중 오류가 발생했습니다.');
            },
            complete: function () {
                $('#loadingMsgRow').hide();
            }
    ...
    function openPanel() {
        ...
        $('#sortOrd').val('0');
        $('#itemTableBody').empty();
        addItemRow({ inputType: 'CHECK', requiredYn: 'N', useYn: 'Y', sortOrd: 1 });
    ...
    function loadChecklistDetail(checklistId) {
        $.ajax({
            ...
            error: function () {
                showMsg('error', '오류', '체크리스트 상세 조회 중 오류가 발생했습니다.');
            }
        });
    }

    function addItemRow(item) {
        const index = $('#itemTableBody tr').length + 1;
        const row = item || {};
        const inputType = row.inputType || 'CHECK';
        const requiredYn = row.requiredYn || 'N';
        const useYn = row.useYn || 'Y';
        const sortOrd = row.sortOrd || index;
    ...
    function renderInputTypeSelect(value) {
        let html = '<select name="inputType" class="form-select">';
    ...
    function saveChecklist() {
        ...
        const hasItem = $('#itemTableBody input[name="itemName"]').filter(function () {
        ...
        $.ajax({
            ...
            error: function () {
                showMsg('error', '오류', '체크리스트 저장 중 오류가 발생했습니다.');
            }
        });
    }

    function deleteChecklist() {
        const checklistId = $('#checklistId').val();
        if (!checklistId) {
            return;
        }
        ...
        showConfirmSwal('삭제', '선택한 체크리스트를 미사용 처리할까요?').then(function (result) {
            ...
            $.ajax({
                ...
                error: function () {
                    showMsg('error', '오류', '체크리스트 삭제 중 오류가 발생했습니다.');
                }
            });
        });
    }
