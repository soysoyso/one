...
- DOCX/HWPX 다운로드 API는 로컬 자동 QA 기준 정상 응답을 확인했다.
- HWPX는 ZIP 기반 PoC 구조로 파일 생성과 다운로드까지 확인했지만, 완전한 한컴 문서 호환 보장 단계는 아니다.
- DOCX는 원본 보고서 양식을 픽셀 단위로 복제한 템플릿이 아니라, 업무 데이터가 들어가는 기본 문서 생성 단계다.
...
2. 기존 PDF 기능은 유지한다.
3. DOCX/HWPX 실제 파일 생성은 기본 다운로드와 데이터 출력까지 완료했다.
4. 특히 HWPX는 일정 리스크가 크므로 한컴 오피스 실호환성 검증과 양식 정밀화는 별도 후속 단계로 둔다.
